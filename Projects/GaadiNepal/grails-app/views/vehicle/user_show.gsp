...
<g:if test="${vehicleInstance.filename}">
    <li class="fieldcontain">
        %{--<span id="picture-label" class="property-label">Picture</span>--}%
...
<g:if test="${vehicleInstance?.brand}">
    <li class="fieldcontain">
        ...
        <span class="property-value" aria-labelledby="brand-label">
            %{--<g:link controller="brand" action="show" id="${vehicleInstance?.brand?.id}">--}%
                ${vehicleInstance?.brand?.encodeAsHTML()}
            %{--</g:link>--}%
        </span>
...
<g:if test="${vehicleInstance?.category}">
    <li class="fieldcontain">
        ...
        <span class="property-value" aria-labelledby="category-label">
            %{--<g:link controller="category" action="show" id="${vehicleInstance?.category?.id}">--}%
                ${vehicleInstance?.category?.encodeAsHTML()}
            %{--</g:link>--}%
        </span>
...
%{--SIMILAR LIST--}%
<g:if test="${similarVehicleList}">
    <h1>Similar Vehicles</h1>
    <table>
        <g:each in="${similarVehicleList}" var="similarVehicle" status="i">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                <g:if test="${similarVehicle.model!=vehicleInstance.model}">
                    <div class="gallery">
                        <g:link controller="vehicle" action="user_show" id="${similarVehicle.id}">
                            <img src="${createLinkTo(dir: 'images/payload/vehicle_pictures/' +
                                    similarVehicle.brand.vec_brand + '/' + similarVehicle.model + '/',
                                    file: '' + similarVehicle.filename)}"
                                 alt="${similarVehicle.model}"
                                 title="${similarVehicle.model}"/>
                        </g:link>

                        <div class="desc">
                            ${fieldValue(bean: similarVehicle, field: "model")}
                        </div>
                    </div>
                </g:if>
            </tr>
